...
import SrcImage from './assets/crow2.png'
import SrcContour from './assets/crow2Contour.png'
...
const WeightedSLR = (img, pos, size) =>{
  ...
  for(i=-halfLength ; i<=halfLength; i++){
    for (j = -halfLength; j<=halfLength;j++){
      ...
      const color = img.GetPixelRGB(new Vector2(pos.x-j, pos.y+i));
      const weight = RGBToGrayScale(color);
      points.push(color);
      const weightC = 1-weight;
      xyw += i*j*weight;
      x2w += (i**2)*weight;
      w += weight;
      xw += i*weight;
...
const createPane = () =>{
  ...
  folder.addBinding(params,'cellSizeX',{min:1, max:50, step:1});
  folder.addBinding(params,'cellSizeY',{min:1, max:50, step:1});
  folder = pane.addFolder({title:'Randomization'});
  folder.addBinding(params,'posJitter',{min:0, max:10, step:1});
  folder.addBinding(params,'widthJitter',{min:0, max:10, step:1});
  folder.addBinding(params,'angleJitter',{min:0, max:1});
  folder.addBinding(params,'lengthJitter',{min:0, max:10,step:1});
  folder = pane.addFolder({title:'Randomization'});
  folder.addBinding(params,'filterSize',{min:5, max:201,step:1});
